...
export default function SavedAlertsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Demo Mode — preview notifikasi sebelum setup */}
      <DemoModeSection permission={permission} setPermission={setPermission} />
...
// ====================================================
// Demo Mode — show user a sample notification preview
// ====================================================

const DEMO_NOTIFICATIONS = [
  {
    title: "🎉 Kupon Shopee Baru!",
    body: "Shopee — Cashback 30% Voucher ShopeePay (max Rp 50.000)",
    code: "SHOPEEPAY30",
  },
  {
    title: "🔥 Tokopedia Promo Spesial",
    body: "Tokopedia — Diskon Rp 100.000 Pengguna Baru (min belanja Rp 500rb)",
    code: "NEWBIE100K",
  },
  {
    title: "💳 OVO Cashback Premium",
    body: "OVO — Cashback Rp 30.000 untuk transaksi pertama",
    code: "OVOFIRST30",
  },
  {
    title: "🚚 Gratis Ongkir Lazada",
    body: "Lazada — Gratis Ongkir Max Rp 25.000 (semua wilayah)",
    code: "FREESHIP25",
  },
];
...
function DemoModeSection({
  ...
  const currentDemo = DEMO_NOTIFICATIONS[demoIndex];

  const fireDemoNotification = async () => {
    if (typeof window === "undefined" || !("Notification" in window)) {
      alert("Browser kamu gak support notifikasi 😢");
      return;
    }

    // Request permission kalo belum granted
    let perm = permission;
    if (perm === "default") {
    ...
    if (perm !== "granted") {
      alert("⚠️ Permission notifikasi belum di-grant. Klik 'Enable Notification' di atas dulu.");
      return;
    }

    // Fire the actual notification
    try {
      const notif = new Notification(currentDemo.title, {
        body: currentDemo.body,
        icon: "/icon-192.png",
        badge: "/icon-192.png",
        tag: "demo-superkupon",
      ...
      setDemoFired(true);
      setTimeout(() => setDemoFired(false), 4000);
    } catch (err) {
      console.error("Demo notif failed:", err);
    }
  };
  ...
  return (
    <section className="rounded-2xl border border-purple-400/30 bg-gradient-to-br from-purple-500/15 via-pink-500/5 to-transparent p-5">
      <div className="flex items-start gap-3">
        <span className="text-3xl">🎬</span>
        <div className="min-w-0 flex-1">
          <h2 className="text-base font-bold text-white">Demo Mode — Lihat Contoh Notifikasi</h2>
          <p className="mt-1 text-xs text-gray-300">
            Penasaran kayak gimana notifikasinya? Klik tombol di bawah untuk preview — 1 contoh
            notifikasi bakal muncul di pojok browser.
          </p>
        </div>
      </div>
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-[1fr,auto]">
        {/* Visual preview card */}
        <div className="rounded-xl border border-white/15 bg-gray-900/60 p-4 shadow-lg">
          <p className="text-[10px] font-bold uppercase tracking-wider text-purple-300">
            👀 Preview Tampilan Notifikasi:
          </p>
          <div className="mt-2 rounded-lg border border-white/10 bg-gray-800/80 p-3">
            <div className="flex items-start gap-3">
              <div className="flex h-10 w-10 flex-none items-center justify-center rounded-md bg-gradient-to-br from-brand-500 to-purple-500 text-lg">
                🎟️
              </div>
              <div className="min-w-0 flex-1">
                <div className="flex items-center justify-between gap-2">
                  <p className="text-xs font-bold text-white">{currentDemo.title}</p>
                  <span className="flex-none text-[9px] text-gray-500">Sekarang</span>
                </div>
                <p className="mt-0.5 text-[11px] text-gray-300">{currentDemo.body}</p>
                {currentDemo.code && (
                  <code className="mt-1.5 inline-block rounded bg-amber-500/20 px-1.5 py-0.5 font-mono text-[10px] font-bold text-amber-200">
                    {currentDemo.code}
                  </code>
                )}
                <p className="mt-1 text-[9px] text-purple-300">SuperKupon</p>
              </div>
            </div>
          </div>
          <button
            type="button"
            onClick={nextDemo}
            className="mt-2 text-[10px] font-semibold text-purple-300 hover:text-purple-200 hover:underline"
          >
            🎲 Ganti contoh ({demoIndex + 1}/{DEMO_NOTIFICATIONS.length})
          </button>
        </div>
        ...
        <div className="flex flex-col items-stretch justify-center gap-2 sm:items-end">
          <button
            type="button"
            onClick={fireDemoNotification}
            disabled={demoFired}
            className="rounded-xl bg-gradient-to-r from-purple-500 to-pink-500 px-5 py-4 text-sm font-bold text-white shadow-lg shadow-purple-500/30 transition hover:scale-105 hover:shadow-purple-500/50 disabled:opacity-60"
          >
            {demoFired ? "✓ Sudah Fire!" : "🔔 Fire Demo Notif"}
          </button>
          <p className="text-center text-[10px] text-gray-500 sm:text-right">
            Aman — cuma 1 notif sekali, gak ngirim ke siapa-siapa.
          </p>
        </div>
      </div>
      ...
      {demoFired && (
        <div className="mt-3 rounded-lg bg-emerald-500/15 px-3 py-2 text-xs text-emerald-200">
          ✅ Demo terkirim! Cek pojok kanan-atas (Windows) atau atas-kanan layar (Mac) untuk lihat
          notifikasi. Nah, itulah yang bakal kamu dapat tiap kali ada kupon baru match dengan
          alert yang kamu save.
        </div>
      )}

      <details className="mt-3 cursor-pointer text-xs text-gray-400">
        <summary className="font-semibold hover:text-white">
          💡 Kenapa pakai Saved Alerts? (klik untuk baca)
        </summary>
        <ul className="mt-2 list-inside list-disc space-y-1 pl-2">
          <li>
            <strong className="text-purple-300">Real-time:</strong> kupon baru muncul = langsung
            di-notif (gak perlu cek manual)
          </li>
          <li>
            <strong className="text-purple-300">Lazy mode:</strong> set sekali, sistem yang
            kerja untukmu di background
          </li>
          <li>
            <strong className="text-purple-300">No miss out:</strong> kupon terbatas (kayak
            "first 100 user") gak kelewat lagi
          </li>
          <li>
            <strong className="text-purple-300">Multi-merchant:</strong> save 10 alert sekaligus,
            track berbagai merchant
          </li>
        </ul>
      </details>
    </section>
